...
#         self.show()
    ...
    def update_sv(self):
        log.debug(tc.FCTCALL + "update BAD16 internal state vector",
                  self.state, tc.ENDC)
        sv_str = ""
        sv_dec = 0
        for i in range(0, 16):
    ...
    def send_state(self):
        addr = 3
        wreg = addr << 25
        log.debug(tc.FCTCALL + "update BAD16 single state vector:", self.state,
                  tc.ENDC)
        ...
        addr = 7
        wregval = addr << 25
        log.debug("BAD16:WREG7: send state vector:",
                  hex(self.vectors[self.state] & 0xffff))
