...
class badChn(QWidget):
    ...
    def __init__(self, parent=None, layout=None, chn=0, cardaddr=3, serialport=None, master=None):
        ...
        if master is None:
            uic.loadUi(os.path.join(os.path.dirname(__file__), 'badchn.ui'), self)
        else:
            uic.loadUi(os.path.join(os.path.dirname(__file__), 'badchn_master.ui'), self)
...
#         if self.unlocked == 1:
    ...
    def send_channel(self):
        log.debug(tc.FCTCALL + "send BAD16 CHN", self.chn, ": index & arrayed register values", tc.ENDC)
        self.send_wreg2()
        self.send_wreg4()
        self.send_wreg5()
