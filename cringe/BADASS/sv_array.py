...
class SV_array(QWidget):
    ...
    def __init__(self, parent=None, seqln=None, addr=None):
        ...
        self.setWindowTitle(
            "PRISM"
        )  # Program for Reconfiguration & Initialization of State Matrices
        ...
        self.globals_layout.addWidget(self.filename_label, 0, 7,
                                      QtCore.Qt.AlignLeft)
        ...
        self.initseq.setToolTip(
            "set all internal state vectors to 0x0000, hit send sequence to send null states to card"
        )
        ...
        self.restore_seq.setToolTip(
            "restore all internal state vectors to values from last load file, hit send sequence to send restored states to card"
        )
        self.globals_layout.addWidget(self.restore_seq, 0, 10,
                                      QtCore.Qt.AlignTop)
        ...
        for i in range(self.nstates):
            if i < seqln:
                self.state_vectors.append(
                    state_vector_builder.state_vector_builder(
                        self,
                        self.array_layout,
                        state=i,
                        enb=1,
                        vectors=self.state_vector_val,
    ...
    def loadSVfile(self):
        log.debug(tc.FCTCALL + "Load state sequence from file: BAD16 /",
                  self.addr, tc.ENDC)
        ...
        if len(self.load_filename) > 0:
            ...
            log.debug(tc.FAIL + "invalid file" + tc.ENDC)

    def fillSV(self):
        if self.load_filename == None:
            log.debug(tc.FAIL + "No file to load/restore states from" +
                      tc.ENDC)
        ...
        for idx, val in enumerate(f):
            if idx < self.nstates:
                ...
                for i in range(0, 16):
                    self.state_vectors[idx].buttons[i].setChecked(
                        bool(int(binstr[15 - i])))
                self.state_vectors[idx].update_sv()
        f.close()
        self.init_flag = 0

    def saveSVfile(self):
        log.debug(tc.FCTCALL + "Save current state sequence to file: BAD16 /",
                  self.addr, tc.ENDC)
    ...
    def initSeq(self):
        log.debug(tc.FCTCALL + "Clear BAD16 state sequence memory: Initialize",
                  self.nstates, "state vectors to 0x0000: BAD16 /", self.addr,
                  tc.ENDC)
...
#           self.state_vectors[i].send_state(1)
    ...
    def SendAllStates(self):
        log.debug(tc.FCTCALL + "Send all", self.nstates, "states: BAD16/",
                  self.addr, tc.ENDC)
    ...
    def packStates(self):
        for idx in range(self.nstates):
            ...
            self.allStates['state%i' %
                           idx] = self.state_vectors[idx].StateVector
