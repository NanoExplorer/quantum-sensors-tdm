...
class TuneTab(QWidget):
    ...
    def unpackState(self, loadState):
        ...
        self.vphidemo.PercentFromBottomSpin.setValue(
        ...
        self.vphidemo.lockSlopeSignFBBCheckBox.setChecked(
            loadState.get("lockSlopeSignFBBCheckBoxChecked", True))
...
class VPhiDemo(QWidget):
    def __init__(self, parent, mm, client):
        ...
        self.button = QPushButton(
            self, text="vphis (changes mix to zero, may change send mode)")
        ...
        self.sendmixcheckbox_label = QLabel(
            "check box to send mix after full tune")
        ...
        layout = QHBoxLayout()
        grabd2abutton = QPushButton(
            self, text="grab d2aA values, assumes feedback is on")
        grabd2abutton.clicked.connect(self.grab_and_set_d2aA_values)
        plotnoisebutton = QPushButton(
            self, text="plot noise (changes nothing)")
    ...
    def _fullTune(self):
        ...
        # choose I for fbb such you get 10 for nsamp = 4, and 5 for nsamp =8
        I = np.ones(len(a2dlockpoints))*int(round(10*4/float(self.mm.NSAMP)))
        ...
        sq1x = np.linspace(
            d2aBupwardSlope[0, 0], d2aBupwardSlope[0, 0]-lfbastats["modDepth"][0, 0])
        sq1y = np.interp(sq1x, fbb2triangle, fbb2sigsup[0, 0])
        plt.plot(sq1x, sq1y, lw=3, label="upward")
        sq1x = np.linspace(
            d2aBdownwardSlope[0, 0], d2aBdownwardSlope[0, 0]-lfbastats["modDepth"][0, 0])
        sq1y = np.interp(sq1x, fbb2triangle, fbb2sigsup[0, 0])
        plt.plot(sq1x, sq1y, lw=3, label="down")
        plt.plot(fbb2stats["firstMinimumX"][0, 0],
                 fbb2stats["firstMinimumY"][0, 0], "o", label="minimum")
        plt.plot(fbb2stats["firstMaximumX"][0, 0],
                 fbb2stats["firstMaximumY"][0, 0], "o", label="maximum")
        if self.lockSlopeSignFBBCheckBox.isChecked():
        ...
        for col in range(self.c.ncol):
            for row in range(self.c.nrow):
                ...
                fbasigsupShifted[col, row,
                                 :] = fbasigsup[col, row, :]-a2d[col, row]
        ...
        for col in range(self.c.ncol):
            for row in range(self.c.nrow):
                if chanisgood[col, row]:
                    ...
                    self.mm.setdfbrow(
                        col, row, a2d=0, I=0, d2aA=minimum_d2aA+0.5*sq1periods[col], d2aB=0, FBA=0, ARL=0)

        sq1periodsstrs = ["col %g %g" % (
            i, int(round(sq1periods[i]))) for i in range(len(sq1periods))]
    ...
    def plotnoise(self):
        ...
        for i in range(navgs):
            data = self.c.getNewData(
                0.001, minimumNumPoints=num_points, exactNumPoints=True)
            for col in range(self.c.ncol):
                for row in range(self.c.nrow):
                    ffts[col, row,
                         :] += np.abs(np.fft.rfft(data[col, row, :, 1]))
        ffts /= float(navgs)
        ...
        noisestr = ["col %d %0.4f" % (i, medpsdcol[i])
                    for i in range(self.c.ncol)]
        log.info("median noise arbs/sqrt(hz) from %0.2f-%0.2f hz:\n" %
                 (freqs[ilo], freqs[ihi])+"\n".join(noisestr))
        log.info(("median across all columns: %g arbs/sqrt(hz)" %
                  np.median(medpsdcol)))
...
class SettlingTimeSweeper(QWidget):
    def __init__(self, parent, mm, client):
        ...
        self.layout.addWidget(QLabel(
            "Settling time sweeps. Run full tune first. Settings only apply to locked."))
    ...
    def sweepUnlocked(self):
        ...
        for col in range(self.c.ncol):
            for row in range(self.c.nrow):
                if row % 2 == 0:
                    ...
                    a2d = adc_hi[col]
                self.mm.changedfbrow(col, row, a2d=a2d, d2aA=8000, tria=0,
                                     trib=0, FBA=1, FBB=0, ARL=0, data_packet=0, dynamic=1)
        ...
        for col in range(self.c.ncol):
            for row in range(self.c.nrow):
                self.mm.changedfbrow(
                    col, row, a2d=0, d2aA=d2aA[col, row], FBA=0)
        ...
        for i, SETT in enumerate(SETTs):
            ...
            data = self.c.getNewData(
                0.01, minimumNumPoints=10000, divideNsamp=False)
        ...
        plots.title("sett sweep, prop delay %g, dfb delay %g, bad delay %g" % (
            self.mm.prop_delay, self.mm.dfb_delay, self.mm.bad_delay))
        plots.xlabel("SETT")
...
class BitErrorDemo(QWidget):
    ...
    def timeouthandler(self):
        data = self.c.getNewData(
            0.001, minimumNumPoints=self.c.sample_rate*1.0)
        ...
        for col in range(self.c.ncol):
            for row in range(self.c.nrow):
                ...
                self.nwrong[col] += len(fbdiff)-np.sum(
                    np.logical_or(fbdiff == 0, fbdiff == self.tristepsize))
                bins = np.arange(
                    self.errormedians[col, row]-self.ehhw, self.errormedians[col, row]+self.ehhw)
        ...
        self.plots.plottitle(
            [": %0.2g/%0.2g" % (self.nwrong[col], self.ntotal[col]) for col in range(self.c.ncol)])
    ...
    def start(self):
        ...
        self.mm.settriangleparams(
            self.tridwell, self.tristeps, self.tristepsize)
        self.mm.setdfball(tria=1)
        ...
        self.hists = np.zeros(
            (self.c.ncol, self.c.nrow, self.ehhw*2-1), dtype="int64")
        for col in range(self.c.ncol):
            for row in range(self.c.nrow):
    ...
    def oneOffVphi(self):
        ...
        for col in range(self.c.ncol):
            for row in range(self.c.nrow):
                ...
                # fb diff should contain only 0s and tristepsize and -tristepsize as values, any other value is an error
                nwrong[col, row] = len(
                    fbdiff)-np.sum(np.logical_or(fbdiff == 0, fbdiff == tristepsize))
            ...
            log.info(("col %g, %g/%g fb values wrong" %
                      (col, nwrongcol[col], nframes*self.c.nrow)))
...
class BiasSweeper(QWidget):
    def __init__(self, parent, mm, client):
        ...
        sweepbutton = QPushButton(
            self, text="Sweep BAD16 high, locked FBA vphis.")
        ...
        sweepbutton = QPushButton(
            self, text="Sweep SQ1b bias, locked FBA vphis.")
...
class ColPlots(QDialog):
    def __init__(self, parent, ncol, nrow):
        ...
        self.figure = plt.figure(
            figsize=(self.numXSubplots*6, self.numYSubplots*4))
        self.canvas = FigureCanvas(self.figure)
    ...
    def plotbigx(self, x, y):
        for col in range(self.ncol):
            ax = self.axes[col]
            for row in range(self.nrow):
                ax.plot(x[col, row, :], y[col, row, :], picker=5,
                        label="col %g, row%g" % (col, row))
        self.draw()
    ...
    def semilogy(self, x, y):
        for col in range(self.ncol):
            ax = self.axes[col]
            for row in range(self.nrow):
                ax.semilogy(x, y[col, row, :], ".", picker=5,
                            label="col %g, row%g" % (col, row))
        self.draw()
    ...
    def semilogx(self, x, y):
        for col in range(self.ncol):
            ax = self.axes[col]
            for row in range(self.nrow):
                ax.semilogx(x, y[col, row, :], picker=5,
                            label="col %g, row%g" % (col, row))
        self.draw()
    ...
    def plotdiffx(self, x, y):
        for col in range(self.ncol):
            ax = self.axes[col]
            for row in range(self.nrow):
                ax.plot(x[col], y[col, row, :], ".", picker=5,
                        label="col %g, row%g" % (col, row))
        self.draw()
