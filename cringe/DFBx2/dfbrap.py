...
class dfbrap(QWidget):
    ...
    def __init__(self,
        ...
        self.glb_var_layout.addWidget(self.MSTR_TX_lbl, 0, 1, 1, 1,
                                      QtCore.Qt.AlignLeft)
        ...
        self.glb_var_layout.addWidget(self.MSTR_RX_lbl, 0, 3, 1, 4,
                                      QtCore.Qt.AlignLeft)
        ...
        self.glb_var_layout.addWidget(self.led_lbl, 0, 7, 1, 1,
        ...
        self.glb_var_layout.addWidget(self.glb_send, 0, 10, 1, 2,
                                      QtCore.Qt.AlignRight)
...
#       self.class_interface_widget.setFixedWidth(self.arrayframe.width()+0)
    ...
    def __str__(self):
        return "dfbrap: slot/addr %g/%g channel %g" % (self.slot, self.address,
                                                       self.col)
    ...
    def FBA_changed(self, state, *bc):
        ...
        if (not (bc) and self.MVTX) == True:
            ...
            if state == 1:
                self.master_vector.FBA_button.setStyleSheet(
                    "background-color: #" + tc.green + ";")
    ...
    def FBB_changed(self, state, *bc):
        ...
        if (not (bc) and self.MVTX) == True:
            ...
            if state == 1:
                self.master_vector.FBB_button.setStyleSheet(
                    "background-color: #" + tc.green + ";")
    ...
    def ARL_changed(self, state, *bc):
        ...
        if (not (bc) and self.MVTX) == True:
            ...
            if state == 1:
                self.master_vector.ARL_button.setStyleSheet(
                    "background-color: #" + tc.green + ";")
            else:
                self.master_vector.ARL_button.setStyleSheet(
                    "background-color: #" + tc.red + ";")
...
#       self.send_wreg4()
    ...
    def MSTR_TX_changed(self):
        ...
        log.debug(tc.FCTCALL + "set Master Vector Broadcast for DFB channel",
                  self.col, ":", bool(self.MVTX), tc.ENDC)
    ...
    def MSTR_RX_changed(self):
        ...
        log.debug(tc.FCTCALL + "set Master Vector Echo for DFB channel",
                  self.col, ":", bool(self.MVRX), tc.ENDC)
...
#           self.mode_button.setText('static')
    ...
    def send_wreg4(self, wreg4):
        if self.parent != None:
            ...
            log.debug(
                "DFB:WREG4: triangle parameters DWELL, STEPS, STEP SIZE (& global relock):",
