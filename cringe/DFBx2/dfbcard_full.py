...
# 		self.dfbx2_widget1.card_delay_changed(self.card_delay.value())
    ...
    def ARLsense_changed(self):
        ...
        log.debug(tc.FCTCALL + "ARL parameter changed:", tc.ENDC)
...
# 		print kHz
    ...
    def tri_idx_changed(self):
        ...
        log.debug(tc.FCTCALL + "triangle timebase changed:", tc.ENDC)
...
# 		self.send_wreg4()
    ...
    def send_triangle(self):
        ...
        log.debug(tc.FCTCALL + "send triangle parameters to both channels:", tc.ENDC)
        self.send_wreg0(1)
        self.send_wreg4()
        self.send_wreg0(2)
        self.send_wreg4()
    ...
    def send_card_globals(self):
        ...
        log.debug(tc.FCTCALL + "send card globals:", tc.ENDC)
        self.send_wreg7()
        

    def send_wreg0(self, col):
        log.debug("WREG0: page register")
        wreg = 0 << 25
        wregval = wreg + (col << 6)
        self.sendReg(wregval)

    def send_wreg4(self):
        log.debug("WREG4: triangle parameters & GR")
        wreg = 4 << 25
        wregval = wreg + (self.tri_idx << 24) + (self.dwell_val << 20) + (self.range_val << 16) + self.GR + self.step_val
        self.sendReg(wregval)

    def send_wreg6(self):
        log.debug("WREG6: global parameters: PS, XPT, ARL, NSAMP")
        wreg = 6 << 25
        wregval = wreg + (self.PS << 24) + (self.XPT << 21) + (self.RLDpos << 16) + (self.ARLsense << 12) +(self.RLDneg << 8) + self.NSAMP
        self.sendReg(wregval)

    def send_wreg7(self):
        log.debug("WREG7: global parameters: LED, ST, delays, sequence length, SETT")
        wreg = 7 << 25
    ...
    def send_GPI4(self):
        log.debug("GPI4: test mode select")
        wreg = 4 << 17
        wregval = wreg + self.TP
        self.sendReg(wregval)

    def send_GPI5(self):
        log.debug("GPI5: test pattern hi-bytes [31..16]")
        if self.TP == 1:
            hibytes = 0x5555
        if self.TP ==2:
            hibytes = 0xaaaa
    ...
    def send_GPI6(self):
        log.debug("GPI6: test pattern lo-bytes [15..0]")
        if self.TP == 1:
            lobytes = 0x5555
        if self.TP ==2:
            lobytes = 0xaaaa
