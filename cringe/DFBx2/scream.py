...
class scream(QWidget):
    ...
    def __init__(self, parent=None, addr=None, slot=None, channel=1, lsync=32):
        ...
        self.glb_var_layout.addWidget(self.led_lbl, 0, 1, 1, 1,
                                      QtCore.Qt.AlignLeft)
        ...
        self.master_vector = dfbChn(self,
    ...
    def triA_changed(self, state):
        ...
        log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "triangle A enable:",
                  state, tc.ENDC)
        GPI = (self.ch - 1) * 24 + 50
        self.send_cmd(GPI, state)
        if state == 1:
            self.master_vector.TriA_button.setStyleSheet(
    ...
    def triB_changed(self, state):
        ...
        log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "triangle B enable:",
                  state, tc.ENDC)
        GPI = (self.ch - 1) * 24 + 51
        self.send_cmd(GPI, state)
        if state == 1:
            self.master_vector.TriB_button.setStyleSheet(
                "background-color: #" + tc.green + ";")
        else:
            self.master_vector.TriB_button.setStyleSheet(
                "background-color: #" + tc.red + ";")
    ...
    def a2d_lockpt_spin_changed(self, level):
        if self.master_vector.lock_button.isChecked() or self.force_cmd == 1:
            ...
            log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "ADC lock point:",
                      level, tc.ENDC)
            GPI = (self.ch - 1) * 24 + 56
    ...
    def d2a_A_spin_changed(self, level):
        if self.master_vector.lock_button.isChecked() or self.force_cmd == 1:
            ...
            log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "DAC A offset:",
                      level, tc.ENDC)
            GPI = (self.ch - 1) * 24 + 57
            self.send_cmd(GPI, level)
    ...
    def d2a_B_spin_changed(self, level):
        if self.master_vector.lock_button.isChecked() or self.force_cmd == 1:
            ...
            log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "DAC B offset:",
                      level, tc.ENDC)
            GPI = (self.ch - 1) * 24 + 58
            self.send_cmd(GPI, level)
    ...
    def data_packet_changed(self, index):
        if self.master_vector.lock_button.isChecked() or self.force_cmd == 1:
            ...
            log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "SEND MODE:", index,
                      tc.ENDC)
            GPI = (self.ch - 1) * 24 + 64
    ...
    def FBA_changed(self, state):
        ...
        log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "FB A enable:", state,
                  tc.ENDC)
        GPI = (self.ch - 1) * 24 + 48
        self.send_cmd(GPI, state)
        if state == 1:
            self.master_vector.FBA_button.setStyleSheet("background-color: #" +
                                                        tc.green + ";")
    ...
    def FBB_changed(self, state):
        ...
        log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "FB B enable:", state,
                  tc.ENDC)
        GPI = (self.ch - 1) * 24 + 49
        self.send_cmd(GPI, state)
        if state == 1:
            self.master_vector.FBB_button.setStyleSheet("background-color: #" +
                                                        tc.green + ";")
    ...
    def ARL_changed(self, state):
        ...
        log.debug(tc.FCTCALL + "SCREAM CH", self.ch, "ARL enable:", state,
                  tc.ENDC)
        GPI = (self.ch - 1) * 24 + 52
        self.send_cmd(GPI, state)
        if state == 1:
            self.master_vector.ARL_button.setStyleSheet("background-color: #" +
                                                        tc.green + ";")
        else:
            self.master_vector.ARL_button.setStyleSheet("background-color: #" +
                                                        tc.red + ";")
    ...
    def send_channel_globals(self):
        ...
        log.debug(tc.FCTCALL + "send channel globals to SCREAM channel",
                  self.ch, tc.ENDC)
        self.GR_changed()

    def GR_changed(self):
        self.GR = self.GR_button.isChecked()
        log.debug("SCREAM channel", self.ch, "global relock enable:", self.GR,
                  tc.ENDC)
        if self.GR == 1:
            self.GR_button.setStyleSheet("background-color: #" + tc.green +
                                         ";")
    ...
    def ENC_changed(self):
        ...
        log.debug("SCREAM channel", self.ch, "encode data enable:", self.ENC,
                  tc.ENDC)
        if self.ENC == 1:
            self.ENC_button.setStyleSheet("background-color: #" + tc.green +
                                          ";")
    ...
    def send_channel(self):
        ...
        log.debug(tc.FCTCALL + "send SCREAM CH", self.ch,
                  "control parameters:", tc.ENDC)
        self.force_cmd = 1
        ...
        self.a2d_lockpt_spin_changed(
            self.master_vector.a2d_lockpt_spin.value())
...
#       self.amp_eng_indicator.setText('%6.3d'%volts)
    ...
    def send_wreg4(self, wreg4):
        if self.parent != None:
            ...
            log.debug(
                "WREG4: triangle parameters DWELL, STEPS, STEP SIZE (& global relock):",
                dwell, steps, step, "(", self.GR, ")")
