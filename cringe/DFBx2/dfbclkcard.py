...
class dfbclkcard(QWidget):
    ...
    def __init__(self, parent=None, addr=1, slot=1, seqln=None, lsync=40):
        ...
        self.setWindowTitle("DFBx1CLK: %d/%d" %
        ...
        self.dfbclk_widget1 = dfbrap.dfbrap(parent=self,
                                            addr=addr,
                                            slot=1,
                                            column=1,
                                            seqln=seqln,
        ...
        self.dfbclk_widget2 = clkrap.clkrap(parent=self,
                                            addr=0,
                                            slot=1,
                                            seqln=seqln,
                                            lsync=lsync)
        ...
        self.dfbclk_widget3 = dprcal.dprcal(ctype="DFBx1CLK",
                                            addr=addr,
                                            slot=slot)
        ...
        self.card_glb_widget.setFixedWidth(
            int(self.dfbclk_widget1.width() / 2 + 10))
    ...
    def seqln_changed(self, seqln):
        log.debug(tc.FCTCALL + "send SEQLN to DFBCLK card:", tc.ENDC)
        self.seqln = seqln
    ...
    def LED_changed(self):
        log.debug(tc.FCTCALL +
                  "send LED boolean (True = OFF) to DFBCLK card:" + tc.ENDC)
...
#        if self.unlocked == 1:
    ...
    def status_changed(self):
        log.debug(tc.FCTCALL + "send ST boolean to DFBCLK card:" + tc.ENDC)
    ...
    def send_triangle(self, wreg4):
        log.debug(
            tc.FCTCALL + "send triangle parameters to DFB CH1 on DFBCLK card:",
            tc.ENDC)
        log.debug("DFBCLK:WREG0: page register: CH 1")
        self.sendReg(1 << 6)
        self.dfbclk_widget1.send_wreg4(wreg4)

    def send_class_globals(self, wreg6, wreg7):
        log.debug(tc.FCTCALL + "send class globals to DFBCLK card:", tc.ENDC)
        self.wreg6 = wreg6
        self.wreg7 = wreg7
        self.send_global_regs()

    def send_card_globals(self):
        log.debug(tc.FCTCALL + "send card globals to DFBCLK card:", tc.ENDC)
    ...
    def send_global_regs(self):
        ...
        log.debug(
            "DFBCLK:WREG6: global parameters: PS, DFBCLK_XPT, CLK, NSAMP:", PS,
            XPT, CLK, NSAMP)
        ...
        log.debug(
            "DFBCLK:WREG7: global parameters: LED, ST, Prop Delay, Card Delay, sequence length, SETT:",
            self.LED, self.ST, PD, CD, SL, SE)
        self.sendReg((self.wreg7 & 0xF3FFFFF) | (self.LED << 23)
                     | (self.ST << 22))
    ...
    def send_dfbclk_wreg6(self):
        if self.parent != None:
            ...
            log.debug(
                "DFBCLK:WREG6: global parameters: PS, DFBCLK_XPT, CLK, NSAMP:",
                self.PS, self.dfbclk_XPT, self.CLK, self.NSAMP)
            self.wreg6 = (6 << 25) | (self.PS << 24) | (
    ...
    def send_dfb_wreg7(self):
        if self.parent != None:
            ...
            log.debug("DFBCLK:WREG7: global parameters: LED, ST, prop delay, dfb delay, sequence length, SETT:", self.LED, self.ST, self.prop_delay, \
...
#       self.sendReg((self.wreg7 & 0xF3FFFFF ) | (self.LED << 23) | (self.ST << 22))
    ...
    def send_GPI4(self):
        log.debug("DFBCLK:GPI4: test mode select")
        wreg = 4 << 17
        wregval = wreg | self.TP
        self.sendReg(wregval)

    def send_GPI5(self):
        log.debug("DFBCLK:GPI5: test pattern hi-bytes [31..16]")
        if self.TP == 1:
            hibytes = 0x5555
        if self.TP == 2:
            hibytes = 0xaaaa
    ...
    def send_GPI6(self):
        log.debug("DFBCLK:GPI6: test pattern lo-bytes [15..0]")
        if self.TP == 1:
            lobytes = 0x5555
        if self.TP == 2:
            lobytes = 0xaaaa
