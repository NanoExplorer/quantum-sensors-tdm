...
class dprcal(QWidget):
    ...
    def __init__(self, ctype=None, addr=None, slot=None):
        ...
        self.globals_layout.addWidget(self.null_phase, 0, 4,
                                      QtCore.Qt.AlignTop)
        ...
        self.globals_layout.addWidget(self.filename_label, 1, 3,
                                      QtCore.Qt.AlignLeft)
        ...
        if ctype == 'BAD16':
            ...
            self.ctr_fct = ("all clocks", "master process clock",
                            "DAC clock 1/2")
        ...
        for idx, val in enumerate(self.enabled):
            ...
            #           self.counters = idx
            pcs = val
            self.phase_counters.append(
                dpr_counter(self,
                            self.array_layout,
                            pcs=pcs,
                            idx=idx,
                            slot=self.slot,
                            coeffs=self.cal_coeffs,
                            appTrim=self.appTrim,
        ...
        #       self.sendSlot()
        self.phase_counters[1].phase_trim_spin.setEnabled(
            0)  # initialize to C0 counter spin disabled 8mar16
    ...
    def loadCalFile(self):
        ...
        if len(filename) > 0:
            ...
            for idx, val in enumerate(f):
                ...
                self.phase_counters[idx].loadCal(
                    int(val)
                )  #   8mar16: chg to call loadCal with passthru coefficient
...
#       if idx != self.counters:
        if (idx + 1) != self.counters:
            log.debug(
                tc.FAIL +
                "WARNING: calibration file does not meet card class format" +
                tc.ENDC)
...
#           self.phase_counters[i].loadCal()
    ...
    def saveCalfile(self):
        ...
        filestring = str(self.card_type) + "_S" + str(self.slot) + "A" + str(
            self.address) + ".txt"
        filename = str(
            QFileDialog.getSaveFileName(self, "save calibration file",
                                        filestring))
        if len(filename) > 0:
            self.filenameEdit.setText(filename)
    ...
    def CalAllCounters(self):
        ...
        log.debug(tc.INIT + "auto phase calibrate: ", self.card_type,
                  "card address", self.address, tc.ENDC)
    ...
    def enbDiagnostic(self, mode):
        ...
        self.phase_counters[1].phase_trim_spin.setEnabled(
            mode)  #   tie C0 spin to enable bit 8mar16
    ...
    def sendReg(self, wregval):
        # Note: pre-refactor sendReg here had no time.sleep after the
        # write. Preserved with sleep_after=0 to avoid behavior change.
        write_wreg(self.serialport, wregval, self.address, sleep_after=0)

    def packCal(self):
        for idx in range(self.counters):
            self.CalCoeffs['coeff%i' % idx] = str(
                self.phase_counters[idx].cal_offset.text())

    def unpackCal(self, CalCoeffs):
        for idx in range(self.counters):
            self.phase_counters[idx].cal_offset.setText(CalCoeffs['coeff%i' %
                                                                  idx])
